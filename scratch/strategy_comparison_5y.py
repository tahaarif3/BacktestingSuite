...
def load_spy_bars():
    data_path = os.path.join("data", "spy_daily_5y.parquet")
    if not os.path.exists(data_path):
        raise FileNotFoundError(f"{data_path} not found. Please run scratch/fetch_spy_5y.py first.")
...
def main():
    print("=== SPY 5-Year Strategy Performance Comparison ===")
    ...
    # Initialize strategies to compare
    strategies = {
    ...
    # Initialize GP strategies
    try:
        gp_default = GeneticProgrammingStrategy("champion_gp.json")
        strategies["GP Strategy (Default Vol Threshold 0.35%)"] = gp_default
        
        # Create a modified GP strategy with daily volatility threshold (e.g. 2.0%)
        class GPAdjusted(GeneticProgrammingStrategy):
            def __init__(self, json_path="champion_gp.json"):
                super().__init__(json_path)
                self.volatility_threshold = 0.02  # 2.0% daily return volatility limit
                
        gp_adjusted = GPAdjusted("champion_gp.json")
        strategies["GP Strategy (Adjusted Vol Threshold 2.0%)"] = gp_adjusted
    except Exception as e:
        print(f"Warning: Could not load GP Strategy: {e}")
    ...
    print("\n=== Performance Comparison Table (5-Year Daily SPY) ===")
